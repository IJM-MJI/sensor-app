...
검증된 방향표로 정렬한 `1_70_2.MOV`와 `1_80_2.MOV`를 포함하고 Recovery 마지막 안정 구간만 Initial로 사용합니다. `1_90_H2O_only_3(response)`와 `1_90_H2O_only_6(response)`는 제공된 정식 타임라인으로 상태·정량 검증에 포함하며, 각각 38초와 32초 이후의 범위 초과 구간은 제외합니다. simultaneous 상태 목표는 명목 RH30--80이며 RH90 reaction은 `saturated/out-of-scope`로 제외합니다. 앱에 배포한 320-tree 직접 4상태 모델의 프레임 정확도는 73.1%, balanced accuracy는 70.6%, 안정 구간 정확도는 83.8%, simultaneous 재현율은 43.3%입니다. 최고 상태 확률 0.35 미만은 `Uncertain / Retake`로 처리합니다. 타임라인과 영상 반응이 충돌하는 구간은 `training/output/simultaneous_review`에 run별 판별 시트로 보존합니다.
...
장소 2의 7범위 RH 모델을 장소 1 H2O-only 상승 run 두 개에 고정한 채 외부 검증한 결과 정확도는 0.250, 인접 범위 허용은 0.417이었습니다. 한 장소 1 run으로 학습하고 다른 run 전체를 제외한 전용 후보도 정확도 0.333, 인접 범위 0.583에 그쳐 배포하지 않습니다. 두 run 모두 보정 색 변화량과 RH의 순위 상관은 높았지만 반응 크기 스케일이 달랐으므로, 장소 2 임계값을 넓히지 않고 장소 1 정량은 추가 독립 run 확보 전까지 범위 밖으로 유지합니다. 재현 가능한 검증은 `training/rh_place1_external_validation.py`와 `training/RH_PLACE1_EXTERNAL_VALIDATION.md`에 기록합니다.

장소 2 내부에서도 response3 전체로 response6을 예측하고 반대 방향을 검증하는 7범위 complete-run holdout을 추가했습니다. 최선의 공용 후보는 정확도 0.444, balanced accuracy 0.448, 인접 범위 0.889로 현재 분리형 앱의 미사용 시점 정확도 0.643보다 낮아 배포하지 않습니다. 같은 두 run의 4개 넓은 범위 검증은 0.875였으므로 거친 RH 변화는 전이되지만 10% 폭의 중간 경계는 run 사이에서 안정적이지 않습니다. 상세 결과는 `training/rh_place2_seven_band_run_holdout.py`와 `training/RH_PLACE2_SEVEN_BAND_RUN_HOLDOUT.md`에 기록합니다.

장소 2의 RH 구간 선정 영향을 별도로 검증했습니다. 기존 단일 프레임 정확도 0.500에서 각 구간의 가장 안정적인 1초 창 중앙값을 사용하면 complete-run held-out 정확도 0.786, 인접 범위 정확도 1.000으로 개선됩니다. 구간 선정이 주요 오차 원인임을 확인했지만 response6 held-out은 0.714이고 40–50% 재현율이 남아 있어 아직 앱에는 배포하지 않습니다. 상세 결과는 `training/rh_place2_stable_window_alignment.py`와 `training/RH_PLACE2_STABLE_WINDOW_ALIGNMENT.md`에 기록합니다.
...
실제 앱 v38 점검에서 3프레임 촬영은 H2 세 지점을 모두 유지했지만 RH 중간 구간은 기존 전역 판별기가 한 단계 낮게 출력했습니다. 검증 당시의 8/8 결과가 환경별 안정 프로토타입과 3프레임의 조합이었다는 점을 반영하여, `sensor-rh-place2-stable-profile-model.js`를 앱에 연결했습니다. 보정 프레임의 `top_a`로 response3/response6 프로필을 자동 선택하며, 선택된 안정 프로토타입 결과는 기존 단일 프레임 수동 band 보정보다 우선합니다.
