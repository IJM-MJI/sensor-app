...
RH는 20%와 30%를 하나의 `20–30%` 구간으로 합치고 40–90%는 10% 단위로 유지합니다. 모든 video-held-out 평가는 테스트 영상의 0% calibration만 허용하고 그 영상의 나머지 프레임 전체를 학습에서 제외하는 `calibration-aware video-held-out`입니다. 전체 H2 데이터(Reaction/hold/Recovery 혼합)의 정확도는 34.6%(stage-balanced 34.7%, ±1% 이내 63.7%)이고, 동일 run의 다른 5초 블록에서는 정확도 64.6%(stage-balanced 54.3%, ±1% 이내 92.3%)입니다. RH endpoint ramp와 최종 RH20 calibration을 적용한 정확도는 36.9%(stage-balanced 35.0%, MAE 12.27%p), 동일 run 5초 블록은 정확도 52.1%(stage-balanced 52.1%, MAE 6.24%p)입니다. 전체 범위 숫자는 앱에서 별도 검증한 뒤 배포합니다.
