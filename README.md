...
RH 카드 배경 제거 A/B에서는 색 거리 상위 15% 마스크가 저농도 신호까지 제거해 실패했습니다. 고정 물방울 템플릿은 exact 22.5%로 기존 23.8%보다 낮았지만 ±1단계 46.5%, MAE 17.00%p로 큰 오차를 줄였습니다. 두 특징의 단순 결합과 예측 앙상블도 exact/balanced를 함께 높이지 못해 배포하지 않았으며, 다음 후보는 calibration 사진에서 구한 미세 회전·이동을 측정 사진에 고정 적용하는 방식입니다.
